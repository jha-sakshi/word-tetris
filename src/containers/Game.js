...
class Game extends Component {
  ...
  _moveLeft = () => {
    ...
    for (let i = 0; i < this.letters.length; i++) {
      if (this.letters[i].moving) {
        if (
          this.letters[i].pos.x > 0 &&
          !this._alreadyHasLetterInPos({
            x: this.letters[i].pos.x - 1,
            y: this.letters[i].pos.y,
          })
        ) {
  ...
  _moveRight = () => {
    ...
    for (let i = 0; i < this.letters.length; i++) {
      if (this.letters[i].moving) {
        if (
          this.letters[i].pos.x < noOfColumn - 1 &&
          !this._alreadyHasLetterInPos({
            x: this.letters[i].pos.x + 1,
            y: this.letters[i].pos.y,
          })
  ...
  _moveDown = () => {
    ...
    for (let i = 0; i < this.letters.length; i++) {
      if (this.letters[i].moving) {
        const alreadyHas = this._alreadyHasLetterInPos({
          x: this.letters[i].pos.x,
  ...
  _checkPossibleWords = () => {
    ...
    this.letters.forEach((_letter, index) => {
      ...
      if (checkWord(possibleWord.toLowerCase()))
        letterEnvolved.forEach((_letter) => (_letter.isWord = true));
    ...
    for (let i = 0; i < noOfColumn; i++) {
      ...
      columns.push(
        <BlockColumn
          ...
          columnId={i}
          letters={letter}
          onLetterClick={this._onLetterClick}
        />
      );
    }
    ...
    return (
      <div className={css(styles.container)}>
        <div className={css(styles.scoreLine)}>
          ...
          <div className={css(styles.score)}> {`Best : 0`} </div>
        ...
        <div className={css(styles.controlContainer)}>
          ...
          {this.wordQueue.length > 0 && (
            <Button
              ...
              size="small"
              color="primary"
              className={css([styles.buttons, styles.destroyColor])}
              onClick={this._checkWordAndDestroy}
            >
              {" "}
              Destroy
            </Button>
